...
class Display:
    ...
    def drawBackground(self, areaName):
        now = utime.localtime(utime.time())
        dst_active = is_dst(now[0], now[1], now[2], now[3])
        dst_label = "DST" if dst_active else "STD"

        timeString = utils.getTimeString()  # just the time
        self.isAsleep = False
        ...
        # Draw area name centered
        self.display.text(areaName, self.getCentreTextPosition(areaName, 2) - 32, 20, 240, 2)

        # Draw time near right side
        time_x = self.width - self.display.measure_text(timeString, 2) - 50  # leave space for DST
        self.display.text(timeString, time_x, 20, 240, 2)

        # Draw DST/STD label at the far right
        dst_x = self.width - self.display.measure_text(dst_label, 2) - 10
        self.display.text(dst_label, dst_x, 20, 240, 2)
